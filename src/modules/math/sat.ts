...
export function computePolygon(poly: IPolygon | number[], mat?: Float32Array): IPolygon | number[] {
    if (poly instanceof Array) {
        ...
        return v2m3(poly, mat as Float32Array)
    }
    const { p, c, e, n } = poly
    for (let i = 0; i < p.length; i += 2) {
        const vec = c[i / 2]
...
function isSeparatingAxis(aPoints: number[][], bPoints: number[][], axis: number[], response?: IResponse): boolean {
    const rangeA = T_ARRAYS.pop() as number[]
    const rangeB = T_ARRAYS.pop() as number[]
...
function pointInCircle(p: number[], c: number[]): boolean {
    const [x, y, r] = c
    const differenceV = T_VECTORS.pop() as number[]
...
function testPolygonCircle(polygon: IPolygon, circle: number[], response?: IResponse): boolean {
    ...
    const circlePos = set(T_VECTORS.pop() as number[], x, y)
    const radius2 = radius * radius
    const points = polygon.c
    const len = points.length
    const edge = T_VECTORS.pop() as number[]
    const point = T_VECTORS.pop() as number[]
    for (let i = 0; i < len; i++) {
